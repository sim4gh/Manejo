...
### Lecciones del smoke testing v1.8.0 (Aramis, 2026-05-12)

1. **JSON + SSH es rescate funcional en emergencia**. Gas capturado mal (rest=0/press=1) por el F8 panel — se parcheó con PowerShell editando `g923_mapping.json` via SSH, sin tener que rebuild ni esperar OTA. Validación clave del pattern v1.7.0/v1.8.0: el JSON serializado es legible humano, parcheable in-situ, y Unity lo carga al próximo boot.
2. **`ReadUnprocessedValue()` retorna 0 si InputSystem no polled el axis en ese frame**. La detección en F8 panel asumía baseline confiable al primer frame del sample 3s — falló para gas porque ese axis no había sido polled antes. Brake y clutch capturaron OK porque ya estaban activos. Lección: para hardware con valores fijos (rest/press hardware-determinados), NO inferir — hardcodear por axis name. v1.8.1 fix.
3. **LAN deploy ≫ Mexicalabs CDN para test kiosks locales**. Aramis está en LAN del operator (Tailscale + 10.0.0.235). SCP directo (~42s) vs Mexicalabs multipart upload (10-15 min) — usar el CDN solo cuando el destino sea remoto productivo. Ver memoria `feedback_aramis_lan_deploy.md`.
4. **No lanzar Unity DX11 games via SSH**. `Start-Process` desde session no-interactiva crashea Unity al `D3D11SetFullscreen`. Pedir al operator lanzar desde console/RDP. Ver memoria `feedback_unity_dx11_needs_console.md`.
5. **Asmdef cycle detection**: `TlaxSim.G923Calibration` referenciaba `Gley.UrbanSystem`, y luego `Gley.UrbanSystem.asmdef` agregó referencia a `TlaxSim.G923Calibration` para permitir `using` en `UIInputNew.cs` → cycle. Fix: G923Calibration NO necesita referenciar Gley (sus clases son data + storage puros). HORI tiene `references=[]` por la misma razón — copiar ese patrón.
6. **TTFE_Drawer scripts**: 4 archivos de `Colonial City Environment Pack` usaban tipos de `UnityEditor` sin Editor folder ni `#if UNITY_EDITOR` → fail en player build (CS0246). Antes funcionaba "por suerte" gracias al cache de Library/ScriptAssemblies. Fix: mover a `Assets/Editor/TobyFredsonDrawers/` (Unity ignora esa carpeta en player builds automáticamente).
