...
{
    // Static singleton: source-of-truth en memoria de la calibración Moto.
    // Solo MotoCalibrationPanel debe llamar Save(). UIInputNew lee Active al
    // boot vía AttachAsMotoSimulator CUANDO flag Moto_UseJsonMapping == 1.
    //
    // Spec: docs/superpowers/specs/2026-05-12-moto-v190-immutable-calibration-design.md
    public static class MotoControlMapping
    {
        public const int CURRENT_SCHEMA = 1;
        public const string FILENAME = "moto_mapping.json";
        public const string PREF_USE_JSON = "Moto_UseJsonMapping";  // 0=legacy, 1=JSON mode

        public static MotoMapping Active { get; private set; }
        ...
        public static void LoadFromDisk()
        {
            ...
            // consumers caen al path PlayerPrefs MOTO_*.
            ...
            if (!File.Exists(path))
            {
                // Primera vez con flag=1: intentar migración desde PlayerPrefs MOTO_*.
                if (MotoMappingMigration.TryMigrateFromPlayerPrefs(out var migrated))
                {
                    Save(migrated);
                    Debug.Log("[MotoControlMapping] Migrated from PlayerPrefs to JSON");
                    return;
                }
                Active = null;
                Debug.LogWarning("[MotoControlMapping] Flag=1 pero no hay JSON y migración falló — Active=null");
                return;
            }
            ...
            try
            {
                ...
                var parsed = JsonUtility.FromJson<MotoMapping>(json);
                if (parsed == null || parsed.schemaVersion != CURRENT_SCHEMA)
                {
                    Debug.LogWarning($"[MotoControlMapping] Schema desconocido {parsed?.schemaVersion}, Active=null");
                    Active = null;
                    return;
                }
                Active = parsed;
                Debug.Log($"[MotoControlMapping] Loaded from {path} (fp={Active.deviceFingerprint})");
            }
            catch (Exception e)
            {
                Debug.LogWarning($"[MotoControlMapping] JSON corrupto en {path}: {e.Message}");
                Active = null;
            }
        }
        ...
        public static void Save(MotoMapping mapping)
        {
            ...
            Active = mapping;
            Debug.Log($"[MotoControlMapping] Saved to {path}");
        }
